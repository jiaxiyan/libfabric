 ...
 * with explicit_refcnt and never has implicit references. This function does
 * not acquire the domain lock because at this point the util_ep is already
 * closed and the EP is no longer reachable from any data path.
 *
 * @param[in]	domain	efa_domain
