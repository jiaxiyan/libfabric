...
{
	/* No internal cq polling. 
	 * Application is responsible for polling the cq to prevent cq overrun. */
}
		     ...
		     struct efadv_comp_cntr_init_attr *efa_cc_attr)
{
	...
	ret = efa_cntr_construct(cntr, domain, attr, efa_hw_cntr_progress, context);
	if (ret) {
