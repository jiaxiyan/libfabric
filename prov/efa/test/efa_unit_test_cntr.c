...
/**
 * @brief Test efa_cntr_open falls back to software counter when efadv_create_comp_cntr fails
 */
void test_efa_cntr_open_sw_cntr_fallback(struct efa_resource **state)
{
	struct efa_resource *resource = *state;
	struct fid_cntr *cntr_fid = NULL;
	struct fi_cntr_attr cntr_attr = {0};
	...
	g_efa_unit_test_mocks.efadv_create_comp_cntr = efa_mock_efadv_create_comp_cntr_return_null_enotsup;

	assert_int_equal(fi_cntr_open(resource->domain, &cntr_attr, &cntr_fid, NULL), FI_SUCCESS);
	assert_non_null(cntr_fid);

	efa_cntr = container_of(cntr_fid, struct efa_cntr, util_cntr.cntr_fid);
	assert_null(efa_cntr->ibv_comp_cntr);
	assert_ptr_not_equal(cntr_fid->ops, &efa_hw_cntr_ops);
...
void test_efa_cntr_open_sw_cntr_fallback(struct efa_resource **state) { skip(); }
