...
struct fi_efa_ops_domain {
	...
	int (*cq_close_ext)(struct fid *cq);
};
```
...
### cq_close_ext
This op closes a completion queue with external memory provided via dmabuf.

#### Return value
**cq_close_ext()** returns 0 on success, or the value of errno on failure
(which indicates the failure reason).
